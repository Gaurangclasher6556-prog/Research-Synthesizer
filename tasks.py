...
def create_planning_task(paper_markdown: str, paper_metadata: dict) -> Task:
    ...
    return Task(
        description=(
            ...
            f"Your task is to generate exactly 3 diverse, high-quality search "
            ...
            f"Format your output as a numbered list of 3 queries. Each query "
            f"should be specific enough to return relevant results but broad "
            f"enough to capture important related work."
        ),
        expected_output=(
            "A numbered list of exactly 3 arXiv search queries, each on its "
...
def create_searching_task(planning_task: Task) -> Task:
    ...
    return Task(
        description=(
            "You have received 3 search queries from the Research Query Planner. "
            "Your task is to:\n\n"
            "1. Execute each query on arXiv using the `arxiv_search` tool\n"
            "2. Collect the paper titles and abstracts from the results\n\n"
            "**Important guidelines:**\n"
            "- Skip papers that fail to download – do not crash\n"
            ...
            "Work through the queries one at a time."
        ),
        expected_output=(
