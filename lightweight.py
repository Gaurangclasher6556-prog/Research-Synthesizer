...
def run_lightweight_pipeline(
    ...
    report_prompt = (
        f"You are an expert AI researcher and academic reviewer, like Andrew Ng. "
        f"Your goal is to critically evaluate this research paper draft and provide actionable, "
        f"copy-pasteable rewrites to reduce plagiarism and make it publishable in a top-tier venue.\n\n"
        f"═══ TARGET PAPER ═══\n"
        f"**Title:** {title}\n**Abstract:** {abstract}\n\n"
        f"═══ RELATED WORK (For Context) ═══\n{related_work_text}\n"
        f"{rag_context}\n\n"
        f"═══ INSTRUCTIONS ═══\n"
        f"Format your response as a polished Markdown document tailored for the authors:\n\n"
        f"# Academic Peer Review & Co-Author Suggestions\n\n"
        f"**Target Paper:** {title}\n\n"
        f"## 1. Strengths & Weaknesses\n"
        f"- **Strengths:** Identify 2 strong points of the current draft.\n"
        f"- **Weaknesses:** Identify the 3 biggest flaws in the methodology, writing, or claims.\n\n"
        f"## 2. Plagiarism Reduction & Strategic Advice\n"
        f"Explain which parts sound generic, copied, or poorly framed. Tell the authors "
        f"exactly how to pivot or reframe their arguments to match the quality of a top AI researcher.\n\n"
        f"## 3. Copy-Pasteable Research Content (The Rewrite)\n"
        f"Provide 2-3 paragraphs of actual, high-quality, deeply original academic text that the authors can "
        f"**copy and paste directly** into their paper to replace their weakest section (like the Abstract or Intro). "
        f"Ground this rewrite using the retrieved context so it is factually sound and distinct from existing work. "
        f"Make it sound highly professional, convincing, and publish-ready.\n\n"
        f"## References for the Rewrite\n"
        f"List any papers from the related work that were used to formulate the rewrite.\n\n"
        f"REQUIREMENTS:\n"
        f"- Target length: ~{target_words} words\n"
        f"- The rewrite section MUST be high-quality academic prose ready for publication.\n"
    )
